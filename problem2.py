...
if __name__ == '__main__':
    ...
    plot3d(ax, y[z==1, 0], y[z==1, 1], vis_bins=30)
    plot3d(ax, y[z==0, 0], y[z==0, 1], vis_bins=30)
    plot3d(ax, y[z==2, 0], y[z==2, 1], vis_bins=30)
    
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    plot3d(ax, *mmr_pred_y[1][:2], vis_bins=30)
    plot3d(ax, *mmr_pred_y[0][:2], vis_bins=30)
    plot3d(ax, *mmr_pred_y[2][:2], vis_bins=30)
    
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    plot3d(ax, *gmm_clsfy_y[1][:2], vis_bins=30)
    plot3d(ax, *gmm_clsfy_y[0][:2], vis_bins=30)
    plot3d(ax, *gmm_clsfy_y[2][:2], vis_bins=30)